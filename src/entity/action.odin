...
action_make :: proc(from, to: u32, type: u32, data: [3]u32) {
    if action_sys.len >= action_sys.cap {
        core.log_error("action_make: too much actions (%d)", action_sys.len)
        return;
    }
    ...
    core.log_debug("make action: from [%d] to [%d]; type = [%d]; data = (%d %d %d)", from, to, type, data[0], data[1], data[2])
}
