...
void effect_sys_init(usize cap) {
    ...
    effect_sys.effect_cap = cap;

    // stub
    effect_sys.effect_head = effect_sys.effect_max_idx = effect_sys.effect_len = 1;
}

// =============================================================================
u32 effect_create(u32 type, effect **r_effect) {
    if (effect_sys.effect_len >= effect_sys.effect_cap) {
        log_err("effect_create(): too much effects => stub");
        assert(false);
        return 0;
    }

    usize idx = effect_sys.effect_head;
    effect *eff = &effect_sys.effect_pool[idx];

    if (idx == effect_sys.effect_max_idx) {
        ++effect_sys.effect_max_idx;
        ++effect_sys.effect_head;
    } else {
        effect_sys.effect_head = eff->pool_flag;
    }

    ++effect_sys.effect_len;
...
void effect_destroy(u32 idx) {
    ...
    eff->pool_flag = effect_sys.effect_head;
    effect_sys.effect_head = idx;
...
[[nodiscard]] effect *effect_get(u32 idx) {
    if (idx == 0 || idx >= effect_sys.effect_max_idx) {
...
void effect_sys_update() {
    for (usize i = 1; i < effect_sys.effect_max_idx; ++i) {
